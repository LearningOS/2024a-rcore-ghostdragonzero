...
use crate::fs::{link_file,unlink_file, open_file,get_inode_type,get_inode_link_number, OpenFlags, Stat};
...
pub fn sys_fstat(fd: usize, st: *mut Stat) -> isize {
    ...
    let task = current_task().unwrap();
    let inner = task.inner_exclusive_access();
    println!("get fstat fd = {}", fd as u32);
    if let Some(file) = &inner.fd_table[fd] {
        let file = file.clone();
        drop(inner);
        ...
        *st = Stat{
            dev: 0,
            ino: inode_id as u64,
            mode: get_inode_type(inode_id),
            nlink: get_inode_link_number(inode_id),
            pad: [0; 7],
        };
        0
...
pub fn sys_unlinkat(name: *const u8) -> isize {
    trace!("kernel:pid[{}] sys_unlink", current_task().unwrap().pid.0);
    let token = current_user_token();
